...
import { IAppDefinition, IIssuerDefinition, IRevokerDefinition, IOrganizationDefinition, IRoleDefinitionV2, IRoleDefinitionText, PreconditionType } from "./types/DomainDefinitions"
import { DID } from "./types/DID";
import { EncodedCall } from "./types/Transaction";
import { VOLTA_RESOLVER_V2_ADDRESS } from "./chainConstants";
import { abi } from "../build/contracts/RoleDefinitionResolverV2.json";

const { namehash } = utils;

export class DomainTransactionFactoryV2 {
  protected readonly _roleDefResolverInterface: utils.Interface
  protected readonly _resolverAddress: string

  constructor({ domainResolverAddress = VOLTA_RESOLVER_V2_ADDRESS }: { domainResolverAddress: string }) {
   ...
   */
  public newRole({ domain, roleDefinition }: { domain: string, roleDefinition: IRoleDefinitionV2 }): EncodedCall {
   ...
   */
  public editDomain({ domain, domainDefinition }: { domain: string, domainDefinition: IRoleDefinitionV2 | IAppDefinition | IOrganizationDefinition }): EncodedCall {
  ...
  protected setRoleDefinitionTx({
    ...
    const textProps = ((roleDef: IRoleDefinitionV2) => {
      return {
