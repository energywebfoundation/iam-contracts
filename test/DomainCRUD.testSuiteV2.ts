...
export function domainCrudTestSuiteWithRevocation(): void {
  describe("Domain CRUD tests for RoleDefinitionResolverV2", () => {
    before(async function () {
      ({
        publicResolverFactory, provider, owner, chainId
      } = this);
    });
    ...
    beforeEach(async () => {
      ...
      domainNotifier = await new DomainNotifier__factory(owner).deploy(ensRegistry.address);
      ...
      await ensRegistry.setSubnodeOwner(rootNameHash, hashLabel(domain2), await owner.getAddress());
    ...
    describe("Role can be created, read and updated", () => {
      const roleCRUDtests = async (role: IRoleDefinitionV2) => {
        ...
        const call = domainDefTxFactoryV2.newRole({ domain: domain2, roleDefinition: role2 });
        ...
        const updateRole = domainDefTxFactoryV2.editDomain({ domain: domain2, domainDefinition: role2 });
      ...
      it('issuer of type "ROLE"', async () => {
        await roleCRUDtests({ ...role2, issuer: { issuerType: "ROLE", roleName: domain2 } });
      });
    });
    ...
    it("app can be created and read", async () => {
      ...
      await ensRegistry.setResolver(node2, ensRoleDefResolverV2.address);
      const domainDefTxFactoryV2 = new DomainTransactionFactoryV2({ domainResolverAddress: ensRoleDefResolverV2.address });
      const call = domainDefTxFactoryV2.newDomain({ domain: domain2, domainDefinition: app });
      await (await owner.sendTransaction(call)).wait()
    ...
    it("org can be created and read", async () => {
      ...
      await ensRegistry.setResolver(node2, ensRoleDefResolverV2.address);
      const domainDefTxFactoryV2 = new DomainTransactionFactoryV2({ domainResolverAddress: ensRoleDefResolverV2.address });
      const call = domainDefTxFactoryV2.newDomain({ domain: domain2, domainDefinition: org });
      await (await owner.sendTransaction(call)).wait()
    ...
    it("domain with unknown resolver type throws error", async () => {
      await ensRegistry.setResolver(node2, '0x0000000000000000000000000000000000000123');
      await expect(domainReader.read({ node : node2 })).to.eventually.rejectedWith(ERROR_MESSAGES.RESOLVER_NOT_KNOWN);
    });
