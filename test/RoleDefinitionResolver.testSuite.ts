...
export function roleDefinitionResolverTestSuite(): void {
  before(async function () {
    ({ ensFactory, domainNotifierFactory, owner, anotherAccount } = this);
  ...
  beforeEach(async () => {
    ...
    roleDefinitionResolver = await new RoleDefinitionResolverV2__factory(
      owner,
    ).deploy(ens.address, domainNotifier.address);
  ...
  describe("upgrading resolver", async () => {
    it("org owner can take over ownership", async () => {
      ...
      const anotherRoleDefResolver =
        await new RoleDefinitionResolverV2__factory(owner).deploy(
          ens.address,
      ...
      await ens
        .connect(roleOwner)
        .setResolver(roleNode, roleDefinitionResolver.address);
      expect(await ens.resolver(roleNode)).to.equal(
        roleDefinitionResolver.address,
      );

      // Confirm that the roleOwner can make some change to the resolver
      await roleDefinitionResolver
        .connect(roleOwner)
        .setText(roleNode, "roleType", "org");
      expect(await roleDefinitionResolver.text(roleNode, "roleType")).to.equal(
        "org",
      );

      // The owner can't set the resolver or update the resolver data
      await expect(ens.setResolver(roleNode, anotherRoleDefResolver.address)).to
        .eventually.be.rejected;
      await expect(roleDefinitionResolver.setText(roleNode, "roleType", "app"))
        .to.eventually.be.rejected;

      // owner can't directly take back the role node as they don't own the org
      await expect(ens.setSubnodeOwner(orgNode, roleLabelHash, ownerAddr)).to
        .eventually.be.rejected;
      ...
      expect(await ens.resolver(roleNode)).to.equal(
        anotherRoleDefResolver.address,
      );
    });
  });
  ...
  describe("domain delegation", async () => {
    it("permits delegation of role creation", async () => {
      ...
      await ens
        .connect(delegate)
        .setSubnodeOwner(orgNode, newRoleLabelHash, ownerAddr);
      await ens
        .connect(delegate)
        .setResolver(newRoleNode, roleDefinitionResolver.address);
      ...
      await roleDefinitionResolver
        .connect(delegate)
        .setVersionNumber(roleNode, newVersionNumber);
      const changedVersionNumber = await roleDefinitionResolver.versionNumber(
        roleNode,
      );
      ...
      await roleDefinitionResolver
        .connect(delegate)
        .setText(roleNode, textKey, newText);
      ...
      await expect(
        ens
          .connect(delegate)
          .setSubnodeOwner(orgNode, anotherRoleLabelHash, ownerAddr),
      ).to.eventually.be.rejected;
      await expect(
        ens
          .connect(delegate)
          .setResolver(anotherRoleNode, roleDefinitionResolver.address),
      ).to.eventually.be.rejected;
      await expect(
        roleDefinitionResolver
          .connect(delegate)
          .setVersionNumber(roleNode, newVersionNumber + 1),
      ).to.eventually.be.rejected;
      await expect(
        roleDefinitionResolver
          .connect(delegate)
          .setText(roleNode, textKey, newText + "changed"),
      ).to.eventually.be.rejected;
    });
  });
  ...
  describe("domain updated notifications", async () => {
    ...
    it("prevents triggering update event directly", async () => {
      await expect(domainNotifier.connect(owner).domainUpdated(roleNode)).to
        .eventually.be.rejected;
    });

    it("prevents triggering update event by non-owner", async () => {
      await expect(
        roleDefinitionResolver.connect(anotherAccount).domainUpdated(roleNode),
      ).to.eventually.be.rejected;
    });
  });
  ...
  describe("issuers", async () => {
    ...
    it("permits setting issuer dids by owner", async () => {
      ...
      expect(issuers.role).to.equal(
        solidityDefaultString,
        "role should be cleared when setting dids",
      );

      const event = await getTransactionEventArgs(tx);
      expect(
        event.args.newIssuers ? event.args.newIssuers[0] : undefined,
      ).to.eql(dids);
      expect(
        event.args.newIssuers ? event.args.newIssuers[1] : undefined,
      ).to.equal(solidityDefaultString);
    ...
    it("permits updating issuer dids by owner", async () => {
      ...
      const moreDids = dids.concat(
        "0xA0B7BD5BFafcEdB39849c9159a2487C0d0d79301",
      );
      await roleDefinitionResolver.setIssuerDids(roleNode, moreDids);
      const issuersWithMoreDids = await roleDefinitionResolver.issuers(
        roleNode,
      );
    ...
    it("permits setting issuer role by owner", async () => {
      const tx = await roleDefinitionResolver.setIssuerRole(
        roleNode,
        anotherRoleNode,
      );
      const issuers = await roleDefinitionResolver.issuers(roleNode);
      expect(issuers.role).to.equal(anotherRoleNode);
      expect(issuers.dids.length).to.equal(
        0,
        "dids should be cleared when setting the role",
      );
    ...
    it("prevents updating issuers by non-owner", async () => {
      await expect(
        roleDefinitionResolver
          ...
          .setIssuerDids(roleNode, dids),
      ).to.eventually.be.rejected;
      await expect(
        roleDefinitionResolver
          .connect(anotherAccount)
          .setIssuerRole(roleNode, anotherRoleNode),
      ).to.eventually.be.rejected;
    });
  });
  ...
  describe("versionNumbers", async () => {
    it("permits setting version number by owner", async () => {
      const initialVersionNumber = await roleDefinitionResolver.versionNumber(
        roleNode,
      );
      expect(initialVersionNumber.toNumber()).to.equal(
        0,
        "version numbers should initialized to 0",
      );
      const newVersionNumber = 1;
      const tx = await roleDefinitionResolver.setVersionNumber(
        roleNode,
        newVersionNumber,
      );
      const changedVersionNumber = await roleDefinitionResolver.versionNumber(
  ...
  describe("issuerTypes", async () => {
    it("permits setting issuer type by owner", async () => {
      ...
      expect(initialType).to.equal(
        0,
        "issuer types should initialized to zero",
      );
      const newType = 1;
  ...
  describe("enrolmentPrerequisiteRoles", async () => {
    it("permits setting prerequisite roles by owner", async () => {
      const initialRoles = await roleDefinitionResolver.prerequisiteRoles(
        roleNode,
      );
      ...
      const tx = await roleDefinitionResolver.setPrerequisiteRoles(
        roleNode,
        newRoles,
        mustHaveAll,
      );
      const changedRoles = await roleDefinitionResolver.prerequisiteRoles(
        roleNode,
      );
    ...
    it("prevents updating prerequisite roles by non-owner", async () => {
      await expect(
        roleDefinitionResolver
          .connect(anotherAccount)
          .setPrerequisiteRoles(roleNode, [], false),
      ).to.eventually.be.rejected;
    });
  });
  ...
  describe("supportsInterface function", async () => {
    it("supports interfaces of new resolver profiles", async () => {
      expect(
        await roleDefinitionResolver.supportsInterface(
          computeInterfaceId("versionNumber(bytes32)"),
        ),
      ).to.be.true;
      expect(
        await roleDefinitionResolver.supportsInterface(
          computeInterfaceId("issuerType(bytes32)"),
        ),
      ).to.be.true;
      expect(
        await roleDefinitionResolver.supportsInterface(
          computeInterfaceId("issuers(bytes32)"),
        ),
      ).to.be.true;
      expect(
        await roleDefinitionResolver.supportsInterface(
          computeInterfaceId("prerequisiteRoles(bytes32)"),
        ),
      ).to.be.true;
      expect(
        await roleDefinitionResolver.supportsInterface(
          computeInterfaceId("domainUpdated(bytes32)"),
        ),
      ).to.be.true;
    });

    it("supports interfaces of resolver profiles from PublicResolver", async () => {
      expect(await roleDefinitionResolver.supportsInterface("0x59d1d43c")).to.be
        .true; //TextResolver
      expect(
        await roleDefinitionResolver.supportsInterface(
          computeInterfaceId("interfaceImplementer(bytes32,bytes4)"),
        ),
      ).to.be.true; // InterfaceResolver
    });

    it("does not support a random interface", async () => {
      expect(await roleDefinitionResolver.supportsInterface("0x3b3b57df")).to.be
        .false;
    });
    ...
    describe("computeInterfaceId function", async () => {
      it("correctly computes known interfaceIds", async () => {
        ...
        expect(computeInterfaceId("supportsInterface(bytes4)")).to.equal(
          "0x01ffc9a7",
        );

        // TextResolver
        expect(computeInterfaceId("text(bytes32,string)")).to.equal(
          "0x59d1d43c",
        );
      });
    });
    ...
    const computeInterfaceId = (contractInterface: string) => {
      return utils
        .solidityKeccak256(["string"], [contractInterface])
